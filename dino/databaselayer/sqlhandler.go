...
func (handler *SQLHandler) UpdateAnimal(a Animal, nname string) error {
	_, err := handler.Exec(fmt.Sprintf(fmt.Sprintf("Update Animals set Animal_type = '%s' , nickname = '%s', zone = %d, age = %d where nickname = '%s'", a.AnimalType, a.Nickname, a.Zone, a.Age, nname)))
	return err
}
